...
int NoAudioCodec::Read(int16_t* dest, int samples) {
    ...
    if (rx_handle_ == nullptr) {
        ESP_LOGE(TAG, "I2S RX channel is not initialized");
        return 0;
    }
...
void NoAudioCodec::EnableInput(bool enable) {
    ...
    if (enable) {
        if (duplex_ && tx_handle_ != nullptr && !output_enabled_) {
            esp_err_t tx_ret = i2s_channel_enable(tx_handle_);
            if (tx_ret != ESP_OK && tx_ret != ESP_ERR_INVALID_STATE) {
                ESP_LOGE(TAG, "Failed to enable I2S TX clock for duplex RX: %s", esp_err_to_name(tx_ret));
                return;
            }
        ...
        if (ret != ESP_OK && ret != ESP_ERR_INVALID_STATE) {
            ESP_LOGE(TAG, "Failed to disable I2S RX: %s", esp_err_to_name(ret));
            return;
        }
    }
...
void NoAudioCodec::EnableOutput(bool enable) {
    ...
    if (enable) {
        ...
        if (duplex_ && input_enabled_) {
            ESP_LOGD(TAG, "Keeping I2S TX enabled as clock source while duplex RX is active");
            return;
        }
