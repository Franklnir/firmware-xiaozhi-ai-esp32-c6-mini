...
bool EspWakeWord::Initialize(AudioCodec* codec, srmodel_list_t* models_list) {
    ...
    std::string configured_model = settings.GetString("wake_word_model", "wn9s_nihaoxiaozhi");
    char *model_name = wakenet_model_->model_name[0];
    for (int i = 0; i < wakenet_model_->num; ++i) {
        ESP_LOGI(TAG, "Wake word model %d: %s", i, wakenet_model_->model_name[i]);
        if (configured_model == wakenet_model_->model_name[i]) {
            model_name = wakenet_model_->model_name[i];
        }
    }
    if (configured_model != model_name) {
        ESP_LOGW(TAG, "Configured wake word model %s is not available, using %s",
            configured_model.c_str(), model_name);
    }
    ...
    if (wakenet_data_ == nullptr) {
        ESP_LOGE(TAG, "Failed to create WakeNet data for %s", model_name);
        return false;
    }
